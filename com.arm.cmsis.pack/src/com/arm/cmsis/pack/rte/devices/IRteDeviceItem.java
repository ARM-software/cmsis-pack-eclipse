...
public interface IRteDeviceItem extends ICmsisMapItem<IRteDeviceItem> {
    ...
    /**
     * Adds device (family, sub-family device or variant) recursively Here we keep
     * the invariant that all the devices are from: <br>
    ...
    /**
     * Returns effective properties of the first device stored in the item or
     * properties with associated processor
     *
    ...
    /**
     * Checks if this item is an end leaf that represent a device that can be
     * selected
     *
     ...
     * @param onlyDevice set to true if only searching for device or variant (no
     *                   family, subfamily)
    ...
    /**
     * Searches device tree for given attributes device name and optional vendor
     * name
     *
