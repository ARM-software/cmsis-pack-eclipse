...
/**
 * Interface to bind CMISIS plug-in with concrete environment. It extends
 * IAdaptable interface to allow querying arbitrary environment-specific
 * interfaces.
 */
public interface ICpEnvironmentProvider extends IRteEventListener, IAdaptable {
    ...
    /**
     * Returns environment name as it is used it CMSIS-Packs, e.g.
     * <code>"uv"</code>, <code>"ds5"</code>, etc.
     *
    ...
    /**
     * Method called from CpPlugIn.start() to initialize the provider and let the
     * provider to register itself for RTE events if needed.<br>
     * The method can also explicitly set environment-specific ICpPackManager or/and
     * ICpPackInstaller to CpPlugIn
     */
    void init();

    /**
     * Method called from CpPlugIn.stop() to let the provider remove its listeners
     ...
     */
    default ICpPackRootProvider getCmsisRootProvider() {
        return null;
    }
     ...
     *
     * @return array of supported names ordered by priority (highest to lowest),
     *         default returns this provider name
     */
    default String[] getSupportedNames() {
        return new String[] { getName() };
    }

    /**
     * Checks if a given environment is supported by this provider. A provider can
     ...
     * @return true if an environment with given name is supported under given
     *         context
     */
    ...
    /**
     * Checks if given example is supported by this provider and can be
     * instantiated.
     *
     ...
     * @return the adaptable object that is created from the example, for instance
     *         IProject
     ...
     */
    default ICpExampleImporter getImporter(ICpExample example) {
        return getDefaultImporter();
    }

    /**
     * Returns default importer to import Eclipse-based examples (those that end on
     * .project in the load path)
     *
     * @return ICpExampleImporter capable to import the example, null no default
     *         importer is supported
     */
    default ICpExampleImporter getDefaultImporter() {
        return null;
    }

    /**
     * Sets default importer to import Eclipse-based examples (those that end on
     * .project in the load path). The purpose of this method is to avoid dependency
     * on com.arm.cmmsis.pack.project plug-in.
     *
     * @param ICpExampleImporter capable to serve as a default, null if no default
     *                           importer is needed
     */
    ...
    /**
     * Get the absolute load (source) path of supplied example, used when copying
     * examples
     *
     * @param example ICpExample to get path from
     * @return load path of this example, or <code>null</code> if example is not
     *         supported
     */
    ...
    /**
     * Get the primary supported environment of supplied example, used when copying
     * examples
     *
     * @param example ICpExample to get environmant from
     * @return primary environment of this example, or <code>null</code> if example
     *         is not supported
     */
    ...
    /**
     * Gets the project folder, i.e., the folder attribute listed in the environment
     * tag.
     *
    ...
    /**
     * Contributes to newly created CMSIS RTE project. It could be an additional
     * project nature, additional files, etc.
     *
     * @param projectName name of the created project, can be used to obtain
     *                    IProject or IRteProject
    ...
    /**
     * Expand key sequences, place-holders and environment variables to those that
     * Eclipse understands
     *
     * @param input      string to expand
     * @param configInfo ICpConfigurationInfo
     * @param bAsolute   boolean flag if to expand to an absolute value (true) or an
     *                   Eclipse variable (false)
