...
/**
 * Extension to ITreeItem interface for trees that use maps for child
 * collections.
 *
 * @param <K> key to be used for sorting (usually String)
 * @param <E> type of items to store in the tree, must implement ITreeItem
 *            interface
 * @see ITreeItem
 */
public interface ITreeMapItem<K, E extends ITreeItem<E>> extends ITreeItem<E> {
     ...
     * @param useFullPath flag indicating if the all path keys should be satisfied
     *                    or only its fraction
     ...
     *
     * @return list of keys for each hierarchy level, the list is empty for the root
     *         item
