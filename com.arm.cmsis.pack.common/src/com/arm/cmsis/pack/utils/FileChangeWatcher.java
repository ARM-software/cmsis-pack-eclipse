...
public abstract class FileChangeWatcher {
    ...
    // flags for watch events (to isolate extender classes from
    ...
    class ActionTask extends TimerTask { // Class to make a delayed (1 sec) processing of change event to allow file
                                         // system to complete
        String fFile;
        int fKind;
     ...
     *
     * @param flags a combination of CREATE, MODIFY and DELETE flags. if 0 the
    ...
    /**
     * Registers file for watching, the file does not need to exists if watcher
     * monitors creation,
     *
     * @param file absolute filename to register for watching, wildcards are allowed
     ...
     *
     * @param file absolute filename to stop watching, must be the same as passed to
     *             regiterFile()
     * @throws IOException
     */
     ...
     * @return 0 if file not watched, 1 if watched explicitly, -1 if implicitly via
     *         wildcards
     */
