...
public interface IMemoryAccess extends IAttributedItem {
	 ...
	 */
	default String getAccessString() { return getAttribute(CmsisConstants.ACCESS);}

	/**
	 * Sets access string
	 * @param access access string to set  
	 */
	default void setAccessString(String access) { updateAttribute(CmsisConstants.ACCESS, normalize(access));}
	 ...
	 * @param access permissions string (contains only "prwx"), co security and privilege
	...
	default boolean isAccessEqual(IMemoryAccess other) {
		...
		if(other == null)
			return false;
		return (isPeripheralAccess()== other.isPeripheralAccess()	&&
				isReadAccess()		== other.isReadAccess()			&& 
				isWriteAccess() 	== other.isWriteAccess()		&& 
				isExecuteAccess() 	== other.isExecuteAccess());
	 ...
	 */
	 default String getAccessMask() {
		...
		if (isExecuteAccess() && !isPeripheralAccess())  { builder.append(EXECUTE_ACCESS); }
