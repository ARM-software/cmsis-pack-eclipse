...
public interface IMemoryPermissions extends IMemoryAccess, IMemorySecurity, IMemoryPriviledge {
     ...
     *
     * @param access permissions string (contains only "prwx"), no security, no
     *               privilege
    ...
    /**
     * Returns combined permissions string in the format
     * access[,security[,privilege]], e.g rwx,s,u
     * <p>
     * Note: the string does not contain peripheral permission 'p' to avoid
     * confusion with peivilege
     *
    ...
    default boolean arePermissionsEqual(IMemoryPermissions other) {
        ...
        if (other == null)
            return false;
        return isAccessEqual(other) && getSecurity() == other.getSecurity() && getPrivilege() == other.getPrivilege();
    }
