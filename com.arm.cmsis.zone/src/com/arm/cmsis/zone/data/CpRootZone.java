...
public class CpRootZone extends CpResourceGroup implements ICpRootZone {
        ...
        for (ICpProcessorUnit processor : processors.values()) {
            if (processor.hasTrustZone()) {
                if (processor.genNumInterrups() <= 0) {
                    deviceUnit
                            .addError(new CmsisZoneError(getResourceFileName(), ESeverity.Error, CmsisZoneError.Z407));
                }
                if (processor.genNumSauRegions() <= 0) {
                    deviceUnit
                            .addError(new CmsisZoneError(getResourceFileName(), ESeverity.Error, CmsisZoneError.Z408));
                }
            }
        }
    }
    ...
    protected void createFtlMemoryElements(ICpItem systemItem) {
        ...
        // Add memory
        for (ICpMemoryBlock r : regions) {
            // Add SAU regions for assigned regions
            ...
            if (setups != null && !setups.isEmpty()) {
                for (ICpPeripheralSetup setup : setups) {
                    ...
                    if (reg_setup == null) {
                        ...
                        reg_setup.addChild(setup.toFtlModel(reg_setup, CmsisConstants.INDEX,
                                setup.getAttribute(CmsisConstants.INDEX)));
