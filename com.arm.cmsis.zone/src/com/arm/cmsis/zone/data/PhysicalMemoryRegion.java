...
public class PhysicalMemoryRegion {
    ...
    protected Set<ICpMemoryRegion> fLogicalRegions = new TreeSet<>(new MemorySizeComparator()); // logical regions with the same physical address, key: ID 
    protected Set<ICpMemoryBlock> fLogicalBlocks = null; // child blocks from all logical blocks in the order of offset/size/name
     ...
     * @param region ICpMemoryRegion to add
     * @return true if added
     */
    public boolean addRegion(ICpMemoryRegion region) {
    	if(region == null)
    		return false;
	...
	public Collection <ICpMemoryRegion> getRegions() {
	...
	public boolean arrangeBlocks() {
		...
		for( ICpMemoryRegion region : fLogicalRegions)  {
	...
	public Long getNextSlot(ICpMemoryBlock blockToAllocate) {
		...
		if(isMpu7Mode() && previousBlock != null) {
			// we are allocating next MPU slot : adjust offset first to the end of previous MPU size
			long alignedOffset = previousBlock.getOffset() + ICpMemoryBlock.getMpu7RegionSize(previousBlock.getSize()); 
			if( offset < alignedOffset) {
				offset = alignedOffset;
			}
			
			// adjust to own alignment
			long alignment = ICpMemoryBlock.getMpu7RegionAlignment(requiredSize);
			alignedOffset = ICpMemoryBlock.alignTo( offset, alignment);
			if( offset < alignedOffset) {
				offset = alignedOffset;
			}
	...
	private Long getAlignedBlockSize(long size) {
		if(isMpu7Mode()) {
			return ICpMemoryBlock.alignToMpu7(size);
		}
		return size;
	}
	...
	protected boolean checkBlock(ICpMemoryBlock block) {
		...
		if(isMpu7Mode()) {
			long alignedSize = ICpMemoryBlock.alignToMpu7(blockSize);
			if(alignedSize > blockSize) {
				block.addError(new CmsisZoneError(ESeverity.Warning, CmsisZoneError.Z172));
    ...
    /**
     * Checks if block properties (permissions, logical address space) match depending on zone mode
