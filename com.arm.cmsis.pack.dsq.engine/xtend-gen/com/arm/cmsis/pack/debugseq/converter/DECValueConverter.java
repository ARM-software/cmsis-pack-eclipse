...
public class DECValueConverter extends AbstractLexerBasedConverter<Long> {
  @Override
  ...
  public Long toValue(final String string, final INode node) {
    ...
    if (_isEmpty) {
      throw new ValueConverterException("Couldn\'t convert empty string to an decimal value.", node, null);
    }
    try {
      long longValue = Long.parseUnsignedLong(string, 10);
