...
/**
 * Responsible for managing toolchain adapters contributed through the extension
 * point
 *
...
public class RteToolChainAdapterFactory {
    ...
    public static final String TOOLCHAIN_ADAPTER_ASSOCIATIONS_EXTENSION_ID = Activator.PLUGIN_ID
            + ".ToolChainAdapterAssociation"; //$NON-NLS-1$
    ...
    /**
     * Returns complete collection of adapters matching supplied toolchain ordered
     * by relevance
     *
    ...
    private void initialize() {
        IExtension[] extensions = Platform.getExtensionRegistry().getExtensionPoint(TOOLCHAIN_ADAPTER_EXTENSION_ID)
                .getExtensions();
    ...
    private void addToolChainAssociations() {
        IExtension[] extensions = Platform.getExtensionRegistry()
                .getExtensionPoint(TOOLCHAIN_ADAPTER_ASSOCIATIONS_EXTENSION_ID).getExtensions();
        for (IExtension extension : extensions) {
            IConfigurationElement[] configElements = extension.getConfigurationElements();
            for (IConfigurationElement config : configElements) {
