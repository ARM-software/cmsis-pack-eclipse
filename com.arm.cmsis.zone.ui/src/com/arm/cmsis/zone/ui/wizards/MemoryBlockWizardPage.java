...
public class MemoryBlockWizardPage extends CmsisZoneAbstractWizardPage<ICpMemoryBlock> {
    ...
    private Label lblLinkerControl;
    private Text textLinkerControl;
     ...
     * Create the wizard.
     */
    public MemoryBlockWizardPage(CmsisZoneController controller, ICpMemoryBlock parentBlock,
            ICpMemoryBlock fExistingItem) {
    ...
    public void createControl(Composite parent) {
        ...
        // 'Linker control' text field
        lblLinkerControl = new Label(container, SWT.NONE);
        lblLinkerControl.setText(Messages.CmsisZoneColumnAdvisor_LinkerControl);
        textLinkerControl = new Text(container, SWT.BORDER);
        textLinkerControl.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 3, 1));
        ...
        container.setTabList(new Control[] { textName, textSize, textOffset, chkAutoOffset, textPhysicalAddress,
                chkStartup, chkNoInit, chkDma, accessControl, textLinkerControl, textInfo });
        container.getShell().setSize(800, 880);
    ...
    private void setInitialValues() {
        ...
        if (block != null) {
            ...
            textLinkerControl.setText(block.getLinkerControl());
...
//		}
    ...
    public boolean apply() {
        ...
        ICpMemoryBlock parentBlock = getParentBlock();
        if (getParentBlock() != null && !parentBlock.isPeripheral()) { // top-level block size and offset cannot be
                                                                       // changed
            Long size = getSize();
        ...
        // Adding linker control to parent element (only memory).
        String linkerControl = textLinkerControl.getText();
        newAttributes.put(CmsisConstants.LINKER_CONTROL, linkerControl);
