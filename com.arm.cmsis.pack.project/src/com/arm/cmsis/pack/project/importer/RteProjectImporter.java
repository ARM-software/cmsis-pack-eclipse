...
public abstract class RteProjectImporter extends PlatformObject implements IRteProjectImporter {
	...
	public void installRequiredPacks(IProgressMonitor monitor)  throws OperationCanceledException, InterruptedException {
		...
		try{
			...
			while (packInstaller.isBusy()) {
				// We have no reference to the progress monitor to notify us when the cancel button is pressed
     ...
     * @throws OperationCanceledException exception thrown  when the user cancels the operation
     ...
     */
    protected IProject createProject(String destinationProjectName, URI destinationURI, String toolchainID, String projectType, IProgressMonitor monitor) throws OperationCanceledException, CoreException {
        ...
        //Add Rte nature to the project before creating CDT project to avoid unnecessary indexing
    ...
    /**
    * Notifies that a given number of work unit of the main task has been completed. Check if this operation should be cancelled
    ...
    /**
     * Deletes project if it already exists in the workspace or directory with the project name exists under destination folder
     */
