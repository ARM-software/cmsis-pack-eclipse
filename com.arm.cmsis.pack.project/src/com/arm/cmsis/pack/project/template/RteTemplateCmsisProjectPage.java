...
public class RteTemplateCmsisProjectPage extends AbstractWizardDataPage implements IStatusMessageListener {
	...
	private RteTooolChainAdapterSelector adapterSelector = null;
	 ...
	 */
	public RteTemplateCmsisProjectPage(String pageName, String title,
	...
	public Map<String, String> getPageData() {
		...
		if(initialized){
			boolean createMain = chkMain.getSelection();
		...
		if(Toutput.equals(CmsisConstants.TOUTPUT_EXE)){
			if(Tcompiler.equals("ARMCC")) //$NON-NLS-1$
				ext = "axf"; //$NON-NLS-1$
			else 
				ext = "elf"; //$NON-NLS-1$
		} else {
	...
	public void createControl(Composite parent) {
		...
		adapterSelector = new RteTooolChainAdapterSelector(mainComposite, SWT.NONE);
			...
			if (cfg != null) {
				...
				if (pt != null) {
					...
					if(artefact != null) {
						...
						if(aId.equals("org.eclipse.cdt.build.core.buildArtefactType.staticLib")) //$NON-NLS-1$
							Toutput = CmsisConstants.TOUTPUT_LIB;
						else if(aId.equals("org.eclipse.cdt.build.core.buildArtefactType.exe")) //$NON-NLS-1$
	...
	public void updateStatus(String message) {
		...
		boolean complete = Tcompiler != null  && !Tcompiler.isEmpty() && 
							adapterId != null  && !adapterId.isEmpty();
