...
public class SequenceView extends ViewPart implements ISelectionListener, IRteEventListener {
    ...
    public void createPartControl(Composite parent) {
        ...
        column1.setLabelProvider(new AdvisedCellLabelProvider(new SequenceViewColumnAdvisor(viewer), COLBUTTON));
    ...
    void updateViewer(IRteProject rteProject) {
        ...
        if (rteConf != null && !viewer.getControl().isDisposed()) {
            engine = CpDsqEngineFactory.create(rteConf.getDeviceInfo(), new RefDebugSeqClient(),
                    SequenceLogger.getInstance(selectedProject));
            if (engine == null) {
                viewer.setInput(null);
                MessageDialog.openError(null, "No Debug Sequence Engine Defined", //$NON-NLS-1$
