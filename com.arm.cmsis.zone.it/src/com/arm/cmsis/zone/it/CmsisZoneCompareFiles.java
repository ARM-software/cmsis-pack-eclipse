...
public class CmsisZoneCompareFiles {
     ...
     * Compares files
     * 
     * @param firstFile:  golden result file
     *                    (workspace\CmsisZoneTest\GoldenResults\TC#\TC#_Device.fzone)
     * @param secondFile: test result file
     *                    (workspace\CmsisZoneTest\TestResults\TC#\TC#_Device.fzone)
     * @return
     * @throws Exception
     */
    public static ArrayList<Set<String>> compareFiles(String firstFile, String secondFile) throws Exception {
        ...
        Scanner file1 = new Scanner(new File(firstFile)); // golden result file
                                                          // (workspace\CmsisZoneTest\GoldenResults\TC#\TC#_Device.fzone)
        Set<String> set1 = getSet(file1);

        Scanner file2Plus = new Scanner(new File(secondFile)); // test result file
                                                               // (workspace\CmsisZoneTest\TestResults\TC#\TC#_Device.fzone)
        ...
        // Removes from set2(test result file) all of its elements that are contained in
        // set1(golden result file)
        plusFiles = compareSets(set1, set2Plus);

        Set<String> minusFiles = new TreeSet<>();
        // Removes from set1(golden result file) all of its elements that are contained
        // in set2(test result file)
