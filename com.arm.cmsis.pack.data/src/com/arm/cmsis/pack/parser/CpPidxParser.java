...
/**
 * Using CpXmlParser this class implements utilities to read files based on pidx
 * schema, e.g. index.pidx, local_repository.pidx.
 *
 * In addition some routines are available to handle specific functionalities
...
public class CpPidxParser {
    ...
    public static final String[] PACK_IGNORE_TAGS = new String[] { CmsisConstants.DEVICES_TAG,
            CmsisConstants.COMPONENTS_TAG, CmsisConstants.BOARDS_TAG, CmsisConstants.CONDITIONS_TAG };
     ...
     *
     * @param pdscFile name of pdsc file
        ...
        if (pidx != null) {
            for (ICpItem item : pidx) {
                ...
                for (ICpItem entry : pdsc) {
                    String packId = entry.getVendor() + '.' + entry.getName() + '.'
    ...
    /**
     *
     * @param item a pair of pack id as key and folder as value
     * @return pdsc file name constructed by folder (not as url, i.e. without
     *         file://localhost/) and pack id
     */
     ...
     *
     * @param localDir .local folder of the current rte
     ...
     *
     * @param xmlFile name of the local repository file
     * @param map     list of pack id mapped to repository folder
     * @param url     current rte root folder
     */
