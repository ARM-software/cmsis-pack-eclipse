...
public class CpItem extends CmsisTreeItem<ICpItem> implements ICpItem {
    ...
    /**
     * Constructs element Id default implementation tries to get "id" attribute if
     * it is not successful, returns <code>"tag.name"<\code> derived classes may
    ...
    public String getAbsolutePath(String relPath) {
        ...
        if (relPath.startsWith("\\\\") || relPath.indexOf(":") == 1) { // Windows only: //$NON-NLS-1$ //$NON-NLS-2$
                                                                       // share or absolute with drive letter
