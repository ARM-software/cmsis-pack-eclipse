...
public class CpDeviceItem extends CpDeviceItemContainer implements ICpDeviceItem {
    ...
    public void addChild(ICpItem item) {
        ...
        if (item instanceof ICpDeviceItem) {
            ...
            if (pname != null && !pname.isEmpty()) {
                children.add(0, item); // add properties with Pname attribute always to front ( makes collecting
                                       // effective properties easier)
            } else {
                children.add(item);
            }
        }
    ...
    protected Object[] createChildArray() {
        // use cached array for effective children, while direct children are rarely
        // needed in GUI
